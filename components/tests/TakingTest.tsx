...
import { 
  ...
  Flag, X, Loader2, CheckCircle2 
} from 'lucide-react';

// Định nghĩa Type cục bộ
...
export default function TakingTest({ 
  ...
  timeLeft, isPaused, togglePause, onSubmit, onExit 
    ...
    // Giữ nguyên HTMLDivElement vì trong code của bạn ref gắn vào thẻ <div> ở cột phải
                        ...
                        {test.questions.map((q, idx) => {
                            ...
                            return (
                                <div 
                                    ...
                                    className="bg-white rounded-2xl shadow-sm border border-slate-200 p-5 md:p-6 hover:shadow-md transition-all"
                                >
                                    {/* Question header */}
                                    ...
                                    <h3 className="text-base md:text-lg font-semibold text-slate-800 leading-relaxed mb-5 whitespace-pre-line break-words">
                                        {q.content}
                                    </h3>
                                        ...
                                        {q.options.map((opt, optIdx) => {
                                            ...
                                            return (
                                                <button
                                                    ...
                                                    className={`w-full p-3.5 rounded-xl text-left border transition-all flex items-start gap-3 group ${
                                                        isSelected 
                                                            ? 'border-green-600 bg-blue-50 ring-1 ring-green-200' // Chọn: Xanh
                                                            : 'border-slate-200 hover:border-slate-300 hover:bg-slate-50' // Thường: Xám
                                                    }`}
                                                >
                                                    <div className={`w-9 h-9 rounded-lg flex items-center justify-center font-bold text-base border shrink-0 transition-all ${
                                                        isSelected 
                                                    ...
                                                    <span className={`text-base font-medium transition-all leading-relaxed pt-1 break-words flex-1 ${
                                                        isSelected 
                                                            ? 'text-blue-900' 
                                                            : 'text-slate-700'
                                                    }`}>
