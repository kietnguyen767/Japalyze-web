...
export default function TakingTest({
  ...
  return (
    ...
    <div className="fixed inset-0 bg-slate-50 z-[1000] flex flex-col h-screen text-slate-800">
      {/* Màn hình Pause */}
      {isPaused && (
              ...
              {test.questions.map((q, idx) => {
                ...
                return (
                  <button
                    ...
                    className={`h-11 rounded-xl font-bold text-sm transition-all border active:scale-95 relative ${isCurrent
                    ...
                    {q.options.map((opt, optIdx) => {
                      ...
                      return (
                        <button
                          ...
                          className={`w-full p-4 rounded-xl text-left border-2 transition-all flex items-start gap-3 group relative overflow-hidden ${isSelected
                        ...
                        >
                          <div className={`w-8 h-8 rounded-lg flex items-center justify-center font-bold text-sm shrink-0 transition-all ${isSelected
                            ? 'bg-blue-600 text-white shadow-md'
                            : 'bg-slate-100 text-slate-500 group-hover:bg-white group-hover:shadow-sm'
                            }`}>
                            {['A', 'B', 'C', 'D'][optIdx]}
                          </div>
                          <span className={`text-base font-medium transition-all leading-relaxed pt-0.5 break-words flex-1 ${isSelected
