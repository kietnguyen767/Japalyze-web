...
// ─── Types ───────────────────────────────────────────────────────────────────
type Quest = (typeof N2_WEEKS)[0]['quests'][0];
type Week = (typeof N2_WEEKS)[0];
...
function ChangeLevelModal({ currentLevel, onClose, onChanged }: {
    ...
    const [selected, setSelected] = useState(currentLevel || 'N2');
...
export default function N2RoadmapPage() {
    ...
    const allOfficialQuestIds = useMemo(() =>
        new Set(N2_WEEKS.flatMap(w => w.quests.map(q => q.id))),
        []);
    ...
    const selectedWeek = N2_WEEKS.find(w => w.week === weekParam) || N2_WEEKS[0];

    const setSelectedWeek = (week: Week) => {
        router.push(`/roadmap/n2?week=${week.week}`, { scroll: false });
    };
    ...
    // ── Helpers ───────────────────────────────────────────────────────────────
    const currentWeek = (() => {
        for (const w of N2_WEEKS) {
            if (!w.quests.every(q => completedIds.has(q.id))) return w.week;
        }
        return N2_WEEKS.length;
    })();

    const totalQuests = N2_WEEKS.flatMap(w => w.quests).length;
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3 md:gap-4">
                {selectedWeek.quests.map((quest: Quest) => {
                    ...
                    const finalLink = isExternal
                        ? `/roadmap/n2/external/${quest.id}?questId=${quest.id}&originalLink=${encodeURIComponent(quest.link)}`
                    ...
                    <div className="bg-white border-b border-slate-200 px-4 pt-4 pb-0 shrink-0">
                        <div className="flex items-center justify-between mb-3">
                            <div>
                                ...
                                <h1 className="text-base font-black text-slate-800">Lộ Trình N2</h1>
                            </div>
                ...
                <div className="hidden md:flex flex-1 min-h-0">
                    <aside className="w-72 shrink-0 flex flex-col border-r border-slate-200 bg-white overflow-hidden">
                        <div className="px-5 py-5 border-b border-slate-100">
                            ...
                            <h1 className="text-lg font-black text-slate-800 leading-tight">Lộ Trình N2</h1>
                            <p className="text-xs text-slate-400 mt-0.5">{N2_WEEKS.length} tuần · Chinh phục Nhật Ngữ</p>
