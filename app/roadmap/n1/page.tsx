//app/roadmap/n1/page.tsx
'use client';

import { N1_WEEKS } from '@/lib/data';
...
// ─── Types ───────────────────────────────────────────────────────────────────
type Quest = (typeof N1_WEEKS)[0]['quests'][0];
type Week = (typeof N1_WEEKS)[0];
...
function ChangeLevelModal({ currentLevel, onClose, onChanged }: {
    ...
    const [selected, setSelected] = useState(currentLevel || 'N1');
...
export default function N1RoadmapPage() {
    ...
    const allOfficialQuestIds = useMemo(() =>
        new Set(N1_WEEKS.flatMap(w => w.quests.map(q => q.id))),
        []);
    ...
    const selectedWeek = N1_WEEKS.find(w => w.week === weekParam) || N1_WEEKS[0];

    const setSelectedWeek = (week: Week) => {
        router.push(`/roadmap/n1?week=${week.week}`, { scroll: false });
    };
    ...
    // ── Helpers ───────────────────────────────────────────────────────────────
    const currentWeek = (() => {
        for (const w of N1_WEEKS) {
            if (!w.quests.every(q => completedIds.has(q.id))) return w.week;
        }
        return N1_WEEKS.length;
    })();

    const totalQuests = N1_WEEKS.flatMap(w => w.quests).length;
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3 md:gap-4">
                {selectedWeek.quests.map((quest: Quest) => {
                    ...
                    const finalLink = isExternal
                        ? `/roadmap/n1/external/${quest.id}?questId=${quest.id}&originalLink=${encodeURIComponent(quest.link)}`
                    ...
                    <div className="bg-white border-b border-slate-200 px-4 pt-4 pb-0 shrink-0">
                        <div className="flex items-center justify-between mb-3">
                            <div>
                                ...
                                <h1 className="text-base font-black text-slate-800">Lộ Trình N1</h1>
                            </div>
                ...
                <div className="hidden md:flex flex-1 min-h-0">
                    <aside className="w-72 shrink-0 flex flex-col border-r border-slate-200 bg-white overflow-hidden">
                        <div className="px-5 py-5 border-b border-slate-100">
                            ...
                            <h1 className="text-lg font-black text-slate-800 leading-tight">Lộ Trình N1</h1>
                            <p className="text-xs text-slate-400 mt-0.5">{N1_WEEKS.length} tuần · Chinh phục Nhật Ngữ</p>
