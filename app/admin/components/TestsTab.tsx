...
import { Plus, Pencil, Trash2, Crown, Clock, FileQuestion, X, Save, Loader2, Lock, Unlock, MessageCircle } from 'lucide-react';
...
export default function TestsTab({ mockTests, setMockTests }: TestsTabProps) {
  ...
  const [questions, setQuestions] = useState<QuestionInput[]>([{ content: '', type: 'vocab', options: ['', '', '', ''], correctAnswer: 0, explanation: '' }]);
  const [activeQuestionIndex, setActiveQuestionIndex] = useState(0);

  // --- 1. Thêm câu hỏi (Bổ sung logic tự động nhảy đến câu mới) ---
  const handleAddQuestion = () => {
    setQuestions([...questions, { content: '', type: 'vocab', options: ['', '', '', ''], correctAnswer: 0, explanation: '' }]);
    setActiveQuestionIndex(questions.length); 
  };

  // --- 2. Xóa câu hỏi (Bổ sung logic validate tối thiểu 1 câu) ---
    ...
    return (
      <div className="fixed inset-0 z-[100] bg-slate-100 flex flex-col">
        ...
        <div className="bg-white px-6 py-4 border-b flex justify-between items-center shadow-sm shrink-0">
          ...
          <button onClick={handleSaveTest} disabled={isSubmitting} className="px-6 py-2.5 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl shadow-md transition-all flex items-center gap-2">
              ...
              {questions.map((q, idx) => (
                <div key={idx} onClick={() => setActiveQuestionIndex(idx)} className={`p-3 rounded-lg cursor-pointer flex justify-between items-center group transition-all ${activeQuestionIndex === idx ? 'bg-blue-50 text-blue-700 font-bold border-blue-100 border shadow-sm' : 'hover:bg-slate-50 text-slate-600'}`}>
                  <span className="text-sm truncate flex items-center gap-2">
                    ...
                    <span className="truncate">{q.content || 'Câu hỏi chưa có nội dung...'}</span>
                  </span>
            ...
            <div className="max-w-3xl mx-auto bg-white p-8 rounded-2xl shadow-sm border border-slate-200">
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <textarea rows={4} value={questions[activeQuestionIndex]?.content} onChange={(e) => updateQuestion(activeQuestionIndex, 'content', e.target.value)} className="w-full p-4 text-lg border rounded-xl outline-none focus:ring-4 focus:ring-blue-100 focus:border-blue-500 transition-all" placeholder="Nhập nội dung câu hỏi (có thể dùng Kanji[furigana])..." />
                </div>
                ...
                <div className="mt-6 bg-blue-50/50 p-5 rounded-xl border border-blue-100">
                  ...
                  <textarea rows={2} value={questions[activeQuestionIndex]?.explanation} onChange={(e) => updateQuestion(activeQuestionIndex, 'explanation', e.target.value)} className="w-full p-3 border rounded-lg text-sm bg-white outline-none focus:border-blue-400" placeholder="Giải thích tại sao đáp án này đúng để học viên nắm vững..." />
                </div>
              </div>
            </div>
          </div>
  ...
  return (
    <div className="space-y-6 animate-fade-in-up">
      <div className="flex justify-between items-center bg-slate-900 text-white p-6 rounded-2xl shadow-lg border-b-4 border-blue-600">
        <div>
          <h2 className="text-2xl font-bold">Kho đề thi JLPT</h2>
          <p className="text-slate-400 text-sm mt-1">Ngân hàng câu hỏi & Đề thi thử trực tuyến</p>
        </div>
        <button onClick={() => { 
          setEditingTestId(null); 
          setTestTitle(''); 
          setQuestions([{ content: '', type: 'vocab', options: ['', '', '', ''], correctAnswer: 0, explanation: '' }]); 
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {mockTests.map(test => (
          <div key={test.id} className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm group relative hover:shadow-xl transition-all">
            ...
            <div className="flex gap-2 mt-5 opacity-0 group-hover:opacity-100 transition-all">
              <button onClick={() => {
                ...
                setQuestions(test.questions && test.questions.length > 0 ? test.questions : [{ content: '', type: 'vocab', options: ['', '', '', ''], correctAnswer: 0, explanation: '' }]); 
                setIsTestFormOpen(true);
                setActiveQuestionIndex(0);
